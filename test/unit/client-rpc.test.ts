...
/** PanelRemoteNamespace 的合法占位：六个方法各给一个 vi.fn()，测试按需覆盖其中几个。 */
function fakeNamespace(overrides: Partial<PanelRemoteNamespace> = {}): PanelRemoteNamespace {
  return {
    snapshot: vi.fn(),
    start: vi.fn(),
    stop: vi.fn(),
...
describe("createPanelApi", () => {
  it("snapshot 成功 → 直接返回 value", async () => {
    ...
    const namespace = fakeNamespace({ snapshot: vi.fn(async () => ({ ok: true as const, value })) });
  ...
  it("start 成功 → 透传形参并返回新 snapshot", async () => {
    ...
    const api = createPanelApi(fakeNamespace({ start }));
  ...
  it("stop 失败 → 抛出 Error 且不吞掉底层 message", async () => {
    ...
    const api = createPanelApi(fakeNamespace({ stop }));
  ...
  it("monitor 成功 → 透传 range/since/signal 并返回 MonitorSnapshot", async () => {
    ...
    const api = createPanelApi(fakeNamespace({ monitor }));
    const controller = new AbortController();
  ...
  it("setDefaultModel 成功 → 透传形参并返回新 snapshot", async () => {
    const value = fakeSnapshot({ defaultModel: "m1" });
    const setDefaultModel = vi.fn(async (model: string) => ({ ok: true as const, value: { ...value, defaultModel: model } }));
    const api = createPanelApi(fakeNamespace({ setDefaultModel }));
    await expect(api.setDefaultModel("m1")).resolves.toEqual({ ...value, defaultModel: "m1" });
    expect(setDefaultModel).toHaveBeenCalledWith("m1");
  });

  it("setDefaultModel 失败 → 抛出携带 code 与 message 的 Error（老面板不支持的场景）", async () => {
    const setDefaultModel = vi.fn(async () => ({
      ok: false as const,
      error: { code: "UNSUPPORTED", message: "当前面板版本不支持默认模型切换（需要面板多模型版本）" },
    }));
    const api = createPanelApi(fakeNamespace({ setDefaultModel }));
    await expect(api.setDefaultModel("m1")).rejects.toThrow(/UNSUPPORTED/);
    await expect(api.setDefaultModel("m1")).rejects.toThrow(/不支持默认模型切换/);
  });
});
