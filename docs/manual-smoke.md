# 手工冒烟手册（dsh Web UI 挂载验证）

> 目的：在真实 dsh Web UI 里验证插件加载、模型选择器出现 llamapad 模型、对话走通、
> 设置卡片正常挂载与派发。自动化 E2E（假面板）已覆盖逻辑正确性；本手册验证「挂载 + 真实链路」。

## 前置

1. **dsh**：两条路都行——
   - **装 CLI**（推荐，实测更省事）：`npm i -g @deepseek-ai/dsh@<版本>`，然后
     `dsh plugin --profile web add /path/to/llamapad-dsh-plugin`（软链装入 web profile），
     配置写进 `$DSH_HOME/profiles/web/cordis.patch.yml`（默认 `~/.dsh`）
   - **克隆源码**：克隆 [deepseek-ai/deepseek-harness](https://github.com/deepseek-ai/deepseek-harness)
     并完成从源码运行的一次性准备（见其 docs 的 quickstart，`pnpm install` 等）；
     只有这条路能用 `--patch` 直接挂 `src/*.ts` 的绝对路径（安装版 CLI 会静默跳过模块路径行）
2. **llamapad 面板**：已部署并可访问；设置页创建一个 API Token（`lp_` 开头）；面板里至少有一个
   模型配置（文件就绪、可启动）
3. 网络：dsh 所在机器能访问面板地址（`panelUrl`）
4. **版本对齐（关键）**：插件 `package.json` 钉的 `@deepseek-ai/dsh-llm` 必须与宿主 dsh 同代。
   构建把 `@deepseek-ai/*` 全部 external，运行时由 pnpm 按插件自己的钉版落盘——版本落后于宿主时
   会装出第二份框架，宿主拿新接口调旧基类而崩（详见下节「真机冒烟结果」的第 ① 条）

## 步骤

1. 复制 `examples/cordis.yml`，改两处：
   - `name:` 改为本仓库 `src/index.ts` 的**绝对路径**
   - `panelUrl:` 改为面板实际地址；`agent-loop` 里的 `model:` 改为面板里实际存在的模型名
2. 在 dsh 仓库根目录启动：

   ```bash
   LLAMAPAD_TOKEN=lp_xxxxxxxxx pnpm dsh web --patch /path/to/llamapad-dsh-plugin/examples/cordis.yml
   ```

3. 打开 `http://127.0.0.1:3080`
4. 在模型选择器里应能看到 llamapad 的模型（显示名 = 面板 displayName，描述带命名空间与量化）
5. 选一个模型发起对话：
   - **默认档（`chatBehavior: strict`，本手册未额外配置时的实际行为）**：聊天路径完全不调
     start/stop。先去 llamapad 面板手动启动这个模型，再在 dsh 发消息——回复应正常流式输出，
     带工具调用的 agent 行为正常，且不会有"冷启动等待"（模型已在跑，请求直接转发）。若选择器里
     选的模型与面板运行中的不一致，或面板当时没有任何模型在跑，插件应**立即报错**而不是等待或
     切换，文案由 `route-message.ts` 的 `formatRouteBlock()` 生成（如"「Y」还没启动，当前运行的
     是「X」。到 设置 → 插件配置 → llamapad 模型面板 里启动「Y」，或改用「X」继续对话"，X/Y
     用的是面板里配置的模型显示名而非配置 key）——这是设计行为，不是 bug
   - **验证冷启动 / 自动切换**：需要在 cordis.yml 里显式加 `chatBehavior: auto-switch` 并重启
     dsh，再重复本步骤：
     - 冷启动：首次响应前会静默等待模型加载（大模型 1-2 分钟以上，受 `startTimeoutMs` 约束，
       默认 300s）——这是物理加载时间，不是插件问题
     - 切换：换选另一个模型再对话，面板里旧容器停止、新容器启动（观察 llamapad 页面可见）
   - **`passthrough` 档**（如需验证）：面板有模型在跑就把请求转发给它，哪怕选择器里选的是别的
     模型名也不会触发切换——回复内容实际来自面板当前运行的那个模型，不代表选择器选中的那个
6. 反向验证（需要 `chatBehavior: auto-switch`）：面板里手动停掉模型，再在 dsh 发消息——插件应
   重新拉起（快路径失效即走完整 ensure）。**默认档（`strict`）与 `passthrough` 档下这一步不成立**：
   面板没有模型在跑时两者都直接报错"还没有模型在运行。到 设置 → 插件配置 → llamapad 模型面板
   里启动「Y」后再发送……"，插件不会替用户按下启动键——同样是设计行为

## 已知边界（截至 A 形态完成时）

- ~~未在真实 GPU 环境跑过~~ **已于 2026-08-25 在 GPU 服务器（RTX 3090，llamapad M4 真机联调环境）完成
  API 层校准**，结果见下节「真机校准结果」。~~dsh Web UI 挂载的端到端冒烟仍待执行~~
  **已于 2026-08-27 完成挂载与派发链路冒烟**，见下节「真机冒烟结果」
- 等待切换期间 dsh 界面无进度提示（静默）——仅 `auto-switch` 档触发 start 时才会有等待；
  `strict`/`passthrough` 档不等待，不满足路由条件时直接报错（见上「步骤」5/6）。静默这一设计
  决策本身：注入文本会污染会话历史上下文，依据见
  [调研文档](research/2026-08-24-dsh-plugin-research.md) §5


## 真机冒烟结果（2026-08-27，dsh 0.1.1-rc.2 / llamapad v0.1.1-rc / 模型 qwen3-1p7b-q8）

装 CLI 那条路走通：`dsh plugin --profile web add <本仓库>` 软链装入，用户层
`cordis.patch.yml` 同时挂 A、B 两个入口，`dsh --profile web --dump-config` 组合树正确、
`dsh --profile web --no-open` 启动无告警。

| # | 验证 | 结果 |
|---|---|---|
| ① | **版本错位复现** | 宿主 `LlmRuntime` 0.1.1-rc.2 + 旧基类 0.0.1-rc.1 的适配器 → `TypeError: registration.adapter.prepareCall is not a function`。新版 `LlmAdapter` 加了 `prepareCall`，宿主派发路径无条件调用它 |
| ② | 依赖对齐后 `prepareCall` | 通过 |
| ③ | `listProviders` / `listModels` | `llamapad` 已注册；列出面板 10 个模型 |
| ④ | **完整对话（`adapterStream`）** | 冷启动首字 10.5s（含模型加载）、总耗时 14.4s；84 个 `text-delta`，`block-start`/`block-end` 各 1、`usage` 1、`finish` 1；`finish.kind = stop`；`usage = {inputTokens:14, outputTokens:89}`；正文正常 |
| ⑤ | B 形态 4 个工具注册 | `llamapad_status` / `llamapad_list_models` / `llamapad_start_model` / `llamapad_stop_model` |
| ⑥ | **工具经真实 `ToolRuntime` 调用** | status（运行中·空闲）→ list（10 个）→ stop 带排空（`排空：idle`）→ status（无模型）→ 再次 stop（`stopped:false` 不报错）→ start `waitReady:false` → status（运行中）。全程 `isError=false`，**输出通过框架的 `output.schema` 真校验**（无 `INVALID_TOOL_OUTPUT`） |

挂载写法有一处坑：B 入口没被任何 bundle 层声明过，用户层必须用 `insert:` 追加，
照抄 A 形态的「按 id 覆盖」写法会得到 `patch: entry "llamapad-tools" not found` 并
**静默不注册**。模板见 `examples/profile-patch.example.yml`。

## 设置卡片冒烟（2026-08-27，dsh 0.1.1-rc.2）

验证第三个入口（`./client`，浏览器端设置卡片）挂载与派发链路。设计与架构见
[设置卡片设计记录](design/settings-card-design.md)。

**前置**：`pnpm run build`——**必须**执行，`package.json` 一旦声明 `dsh.client`，
宿主找不到 `dist/client.js` 会让整个插件（连 A 形态一起）加载失败（见[开发与调试](guide/zh/development.md#先构建再启动-dsh)）。

**启动**：

```bash
cd /root/.dsh/profiles/web
LLAMAPAD_TOKEN=lp_xxxxxxxxx nohup dsh --profile web > /tmp/dsh-web.log 2>&1 &
```

dsh 监听 `http://127.0.0.1:3080`。

**不开浏览器也能验的三条**（用 `curl --noproxy '*'`）：

1. 产物被宿主路由：

   ```bash
   curl -s --noproxy '*' -o /dev/null -w '%{http_code}\n' \
     http://127.0.0.1:3080/plugins/llamapad-dsh-plugin/client.js
   curl -s --noproxy '*' -I http://127.0.0.1:3080/plugins/llamapad-dsh-plugin/client.js | grep -i content-type
   ```

   应 200，`content-type: text/javascript`（实测 `text/javascript; charset=utf-8`）。

2. 进了 boot 图：

   ```bash
   curl -s --noproxy '*' http://127.0.0.1:3080/ | grep -o '"id":"llamapad-dsh-plugin"'
   ```

   `/` 的 HTML 里 `__DSH_BOOT__` 的 entries 应含 `"id":"llamapad-dsh-plugin"` 这一条，能匹配到即算通过。

3. host RPC 通：

   ```bash
   RPCID=$(cat /proc/sys/kernel/random/uuid)
   curl -s --noproxy '*' -X POST http://127.0.0.1:3080/api/llamapadPanel/snapshot \
     -H 'content-type: application/json' \
     -d "{\"type\":\"client-request\",\"rpcId\":\"$RPCID\",\"method\":\"llamapadPanel/snapshot\",\"payload\":{\"args\":{}}}"
   ```

   应返回 `{"type":"server-response","rpcId":"<同一个>","result":{"ok":true,"value":{...CardSnapshot...}}}`。
   用无效 token 时 `panelError` 应是「llamapad token 无效或未授权，请检查插件配置里的 token」——
   实测确认，这条同时验证了错误路径（面板不可达/鉴权失败不抛错，塞进 `panelError` 由卡片显示）。

**浏览器验证**：dsh 开屏有一层「内测声明」引导弹层，遮罩会拦截点击，先点「继续」关掉；再点
侧栏「设置」→「插件」→「插件配置」，卡片在官方三张卡（终端 / Agent 循环 / 网页搜索）下方。

## 卡片交互与连接配置验收清单（2026-09-02，待执行）

验证本轮新增的三块行为：卡片折叠 + 模型列表双列定高滚动、选择器只显示运行中模型 + 运行标记
改播放三角、聊天被拦报错改操作引导（以上四条自动化测试覆盖不到组件与选择器本身）；以及卡片内
直接填写面板地址与 token、改完即时生效不用重启 dsh。自动化测试（假面板 E2E）已覆盖判定逻辑
本身，本清单只验真实 dsh + 真实面板下的挂载与交互。

**卡片折叠、列表布局与选择器**：

1. 设置 → 插件配置：llamapad 卡片默认展开，点标题栏能收起、chevron 转 180°，再点展开
2. 收起状态下观察一分钟，面板侧不应再有新的轮询请求（可看面板访问日志）
3. 模型列表一行两个，超过约 4 行时列表内部出现滚动条、卡片本身不被撑高
4. 把 dsh 窗口拖窄，列表回落成单列
5. 模型选择器里运行中的模型前缀是 `▶︎`（文本形态的三角，不是彩色 emoji）
6. 打开 `hideStoppedModels` 后重启 dsh，选择器只剩运行中的那个模型
7. `chatBehavior: auto-switch` + `hideStoppedModels: true` 时启动 dsh，控制台有一条忽略警告，
   且选择器仍列出全部模型
8. strict 档下选一个未启动的模型发消息，报错文案是新的操作引导，且模型名是面板里配的中文名

**卡片内连接配置**：

1. 把 cordis.yml 里的 `panelUrl`/`token` 清空重启 dsh → 设置页里卡片**仍然出现**，显示未连接态
2. 在卡片里填地址与 token → 保存 → **不重启**，卡片在下一次轮询（≤5s）内显示出模型列表
3. 保存后检查 `$DSH_HOME/settings.yaml`，应出现 `llamapad-panel:` 分节且含 `panelUrl`/`token`；
   `cordis.yml` / `cordis.patch.yml` 一字未改
4. token 输入框留空、只改地址 → 保存 → 原 token 仍然有效（不被清空）
5. 地址填空白 → 保存按钮禁用并给出原因
6. 保存后立刻发一条对话 → 走的是新地址（可在新面板的访问日志里确认）
7. 浏览器 DevTools 的 network 里检查 snapshot 响应体，**不应出现 token 明文**

## 2026-09-03 第三批功能冒烟（待执行）

验证第三批落地的六块行为：贴图对话、取消等待、事件流、监控页、提示词快照、审批门。
自动化测试（497 单测 + 31 假面板 E2E）已覆盖逻辑本身，本节只验真实 dsh + 真实面板 / GPU
下的挂载与链路。**本轮尚未真机验证，以下条目为待执行清单。**

**1. 贴图对话**：

- 步骤：选一个配了 mmproj 的视觉模型发起对话，贴一张 PNG 并附一句话（如「描述这张图」）；
  再换一个没配 mmproj 的文本模型重复贴图发送
- 预期：视觉模型能描述图片内容；文本模型不开放贴图（inputModalities 门控），消息里带图
  （旧会话/工具结果）时降级为显式占位文本，不报错

**2. 取消等待**：

- 步骤：在设置卡片里启动一个较大的模型（或对正在推理的模型点停止），等待期间点「取消等待」
- 预期：按钮恢复成原样（启动/停止），无红色错误横幅（主动取消不是故障），下一次操作可正常发起

**3. 事件流**：

- 步骤：在 llamapad 面板侧启停一个模型，观察 dsh 设置卡片与模型选择器；再 kill 掉模型容器
  （或在推理中停模型）
- 预期：卡片「最近事件」出现对应条目（`model.start` 等绿色）；选择器 `▶︎` 标记秒级跟随
  （事件驱动，不再是轮询间隔级别的延迟）；容器被 kill 后出现红色 `model.exit` 条目并弹 Toast

**4. 监控页**：

- 步骤：打开 dsh 设置，确认左侧导航出现 llamapad 监控页；进入后切 30m/2h/24h/7d 四档；
  发起一次对话再回看监控页
- 预期：六条曲线（tokens/s、KV cache、GPU 显存/利用率、容器 CPU/内存）有数据，聊天期间
  tokens/s 曲线爬坡；GPU 分卡明细的温度/功耗有值（nvidia-smi 可报时）；切档正常，离开页面
  后不再轮询

**5. 提示词快照**：

- 步骤：任选一个模型（含云端 provider 的）问「本地有什么模型在跑 / 可以启动」；或 dump
  系统提示核实
- 预期：答案与面板实际状态一致；系统提示里存在 `llamapad:local-fleet` 分节（英文，running +
  可启动清单）；关闭 `statusPromptSection` 后该分节消失

**6. 审批门**：

- 步骤：B 形态配置 `toolApproval: ask` 并重启 dsh，在对话里让 Agent 调 `llamapad_start_model`
  （如「帮我启动模型 X」）
- 预期：工具执行前弹出用户确认；确认后正常执行，拒绝则不执行（宿主无审批通道时同样不执行）

## 多模型并行适配冒烟（2026-09-21，待执行——需面板 `feature/multi-model` 分支合并进 dev/main 后才能验证）

本插件提前适配了 llamapad 面板尚未合并的 `feature/multi-model` 分支（解除「同一时刻只
运行一个模型」的约束，计划见 `docs/plans/2026-09-21-multi-model-adapt.md`）。自动化测试
（618 单测 + 36 假面板 E2E，含假面板「多模型模式」与「老面板模式」两套契约）已覆盖逻辑
本身；本节只验真实多模型面板 + 真实 dsh 下的挂载与链路，**目前面板分支未合并，以下条目
全部为待执行清单，不打勾**。

**1. 多模型并行运行 + 选择器标记**：

- 步骤：在面板上先后启动两个模型 A、B（不停 A 就直接起 B）
- 预期：dsh 模型选择器里 A、B 都显示为运行中（`▶︎` 前缀）；先启动的那个（面板"起第一个
  模型自动成为默认"的行为）额外带 `★` 前缀，说明文字追加「默认（不指定模型的请求发给它）」

**2. auto-switch 请求非默认模型（P0-1 真机复现）**：

- 步骤：`chatBehavior: auto-switch`，面板已有默认模型 A 在跑，dsh 对话里选模型 B 发起对话
- 预期：B 正常就绪并开始流式回复，**不会**等到 `startTimeoutMs`（默认 300s）才假超时；
  对话结束后确认 A 仍在面板上运行（auto-switch 不驱逐其他模型）

**3. strict 对非默认模型对话（P0-2 真机复现）**：

- 步骤：`chatBehavior: strict`（默认档），面板已同时跑着 A（默认）与 B（非默认），dsh 对话
  里选模型 B 发起对话
- 预期：请求正常转发给 B 并得到回复，**不报** `MODEL_NOT_RUNNING`（旧实现会把"运行的不是
  默认模型"误判成"目标未运行"）

**4. 设置卡片：多模型运行列表 + 「设为默认」**：

- 步骤：面板同时跑两个模型时打开设置卡片；点非默认行的「设为默认」按钮
- 预期：运行区从单行变成列表（每行模型名 · 已加载时长 · ready 状态点 · 默认标记/按钮）；
  点击「设为默认」后按钮变禁用态、成功后默认标记切换到新行；停掉一个模型后列表同步减一行，
  只剩一个模型时运行区回落成单行视觉（不因为多模型改造走样）

**5. `llamapad_set_default_model` 工具**：

- 步骤：B 形态里让 Agent 调用该工具切换默认模型（如「把默认模型切到 B」）；随后发一条不带
  模型名的对话请求
- 预期：切换成功；后续不带模型名的请求确实打到新默认模型 B；对没在运行的模型调用应收到
  面板的 409 说明（而不是被吞成成功）

**6. 监控页归属标注**：

- 步骤：面板同时跑两个模型时打开 dsh 监控页；把默认模型从 A 切到 B 后观察曲线
- 预期：容器/推理两张卡顶部出现「容器与推理指标来自默认模型 X」提示，X 随默认模型切换
  实时更新；切换默认模型后曲线在切换点直接拼接新模型的数据（**没有边界标记，这是面板采集器
  本身的约束，不是插件 bug**）；GPU 卡出现「整卡口径」提示

**7. 系统提示快照多模型文案**：

- 步骤：面板同时跑两个模型时，任选一个模型（含云端 provider）dump 系统提示或直接问「本地
  有什么模型在跑」
- 预期：`llamapad:local-fleet` 分节列出全部在跑模型（都带 quant/context 标注，默认模型
  额外标 `default`），并有一句「不指定模型的请求会发给谁」；可启动清单里两个在跑模型都不
  出现（而不是只剔除默认那一个）

**8. 老面板兼容回归**（若还能找到未合并该分支的旧版面板环境）：

- 步骤：接一个老面板（无 `models[]`/`defaultModel`/`default-model` 路由），重复走一遍
  `docs/manual-smoke.md` 前面几节的既有冒烟步骤
- 预期：行为与本次适配之前完全一致——单模型选择器标记、`strict`/`passthrough`/`auto-switch`
  三档、卡片单行运行态、`llamapad_set_default_model` 报「当前面板版本不支持默认模型切换」

## 双版本冒烟（2026-09-24，dsh 0.1.5-rc.3 / 0.1.7-rc.1）

验证本轮双版本兼容改造（方案见 `docs/plans/2026-09-24-dsh-dual-compat.md`）：框架依赖从钉精确
版本改为带范围的 peerDependencies，运行时用宿主那一份框架；两代差异收在设置服务、RPC codec、
工具结果消息、图标、设置卡片插槽五个兼容点。逻辑本身由单测、假面板 E2E 和 `test:compat` 的
0.1.5 矩阵覆盖，本节只验真实宿主下的挂载与交互。

做法：`pnpm run build && pnpm pack`，用 tgz 装进 web profile（走真实的 peer 解析路径），
`dsh web --no-open` 后在浏览器里逐项核对；0.1.5 用 `npm i -g @deepseek-ai/dsh@0.1.5-rc.3`
切换 CLI，profile 的框架包是指向 CLI 自带依赖的软链，跟着一起切，插件不用重装。

| # | 检查项 | 0.1.7-rc.1 | 0.1.5-rc.3 |
|---|---|---|---|
| 1 | 插件加载成功，无 `Failed to load plugins`，控制台无本插件报错 | ✅ | ✅ |
| 2 | 模型选择器列出面板上的模型 | ✅ 3 个模型，运行中的默认模型带 `▶︎ ★` | ✅（headless，见下） |
| 3 | 设置卡片出现 | ✅ 侧栏「插件」→ llamapad-dsh-plugin 详情页 | ✅「设置 → 插件 → 插件配置」 |
| 4 | 卡片保存连接写入正确位置，token 留空时保持原值，插件不重启 | ✅ 写进 profile `cordis.patch.yml` 的 llamapad 行 | ✅ 写进 `$DSH_HOME/settings.yaml` 的 `llamapad-panel` 节，`cordis.patch.yml` 未动 |
| 5 | 设置导航有「llamapad 监控」页 | ✅ | ✅ |
| 6 | 对话流式返回 | ✅ Qwen3.5-4B，3 秒完成，约 97 tok/s | ✅（headless，见下） |

第 2、6 项在换上有效 token 后补测（同日）。0.1.7 在 web 界面里完成：卡片里保存新 token 后立即
列出面板模型，用卡片的「启动」拉起 Qwen3.5-4B，选择器里选中它发消息，正常流式返回。

0.1.5 的界面在共用 `~/.dsh` 时读不了 0.1.7 写过的工作区状态，停在「选择一个工作区开始」，
无法发消息（宿主问题）。改用一个干净的 `DSH_HOME` 装同一个 tgz，走 `dsh --profile headless`
（0.1.5 还不支持 `dsh headless` 简写）：默认模型指向 4B 时正常回复；指向没在跑的 9B 时返回
`MODEL_NOT_RUNNING`，并准确说出当前在跑的是 4B。这说明请求确实经过本插件，插件也拿到了面板
的模型列表。`link:` 本地调试模式在 0.1.7 下同样能正常加载。

补测时发现并修掉一个既有 bug：`sharedModelGate` 按面板地址缓存启停门，并永远绑着第一次传入
的 client。面板地址不变、只在卡片里改了 token 时，列模型用新 token，启停却还拿旧 token，结果
报 401，卡片上看起来像「点了启动没反应」。现在门会改用最近一次传入的 client。

还有一处观察，留待多模型面板的真机冒烟一起看：模型在加载、尚未就绪的那几分钟里，卡片一直显示
「当前没有模型在运行」，按钮也没有变成「取消等待」；就绪后才显示为运行中。

实测踩到的两件事：

- **从 `link:` 切到 tgz 安装前，先删掉 profile 里的旧插件目录**
  （`rm -rf ~/.dsh/profiles/web/node_modules/llamapad-dsh-plugin`）。直接 add 的话，旧软链目标下
  的开发依赖会残留在插件目录的嵌套 `node_modules` 里（含 `@deepseek-ai/*` 0.1.7），插件运行时会
  优先解析到这份而不是宿主那份，peer 改造等于白做。干净安装不会出现嵌套目录。
- **在 0.1.7 用过之后再切回 0.1.5，测完要删掉 0.1.5 新生成的 `~/.dsh/settings.yaml`**。否则下次
  0.1.7 启动会再导入一次，并把它改名成 `settings.yaml.imported`，覆盖掉第一次导入时留下的那份
  原始备份。0.1.5 侧栏的工作区组件在读 0.1.7 写过的状态时会报 `retainAccountKeys` 错误，属于宿主
  自身问题，与本插件无关。

## 启动中状态冒烟（2026-09-24，llamapad dev 6bbb5bd / dsh 0.1.7-rc.1）

背景：模型加载的那几分钟里，卡片一直显示「当前没有模型在运行」，启动按钮约 70 秒后又变回「启动」，还提示面板不可达。
读面板源码后确认：start 接口要做完「校验 → 清旧容器 → 建容器（本地没有镜像时先拉取）→ 启动 → 10 秒存活检测」才返回。
拉镜像期间容器还不存在，`runtime/status` 里看不到这个模型。插件对 start 请求只等 70 秒，超时后又被当成「面板不可达」。
面板因此新增了 `starting` 字段，插件把启动请求的超时放宽到 5 分钟，超时后改为「仍在处理」，卡片显示「仍在启动中」。

**构造拉镜像窗口**：给模型用的 llama.cpp 镜像打一个备份 tag，再删掉原 tag。启动时面板找不到镜像，会走拉取路径。测完再把原 tag 恢复，删掉备份 tag。
本机的镜像层都还在，所以这次拉取大约 1 秒就结束了。

面板 API（0.3 秒轮询 `runtime/status`，启动 4B）：

| 时间 | starting | models |
|------|----------|--------|
| 0s | 4B · preparing | 空 |
| 0s | 4B · pulling | 空 |
| 2s | 4B · creating | 4B ready=false |
| 4s | 4B · creating | 4B ready=true |
| 12s（start 返回） | 空 | 4B ready=true |

- [x] 三个阶段依次出现，start 返回后 `starting` 清空
- [x] 容器建好后大约有 8 秒，模型同时出现在 `models` 和 `starting` 里。卡片对已进入 `runningModels` 的模型不再显示「仍在启动中」，这条规则就是这次实测后加上的
- [x] dsh 卡片从点击到运行，没有出现「面板不可达」横幅；「刷新」按钮显示正常
- [x] 首次实测时发现，点击后到第一次轮询之间，卡片仍显示「当前没有模型在运行」。已改为：有启停请求在途时，立即拉一次状态，之后按 2 秒轮询（b65d7da）
- [ ] 真实的几分钟级拉取没有测：需要删掉整个镜像（约 7GB）后重新下载，代价太大。「仍在启动中」这一行的渲染和 5 分钟超时路径，由单测和假面板 E2E（`test/e2e/starting-e2e.test.ts`）覆盖
- 注：`startRequestTimeoutMs` 取三个值中的最大值，调小不会让请求提前超时（排空开启时下限是 70 秒），所以真机上没法用调小这个值的办法去触发超时路径

## 真机校准结果（2026-08-25，llamapad v0.1.0-rc / RTX 3090 / Qwen3.6-35B-A3B）

用 `lp_` token 直接打面板 API 复现插件的调用链，校准 A 形态遗留的三项：

**控制面**（`PanelClient` 的 5 个方法 + 就绪探测，全部 Bearer 鉴权）：

| 端点 | 结果 |
|---|---|
| `GET /api/v1/models` | 200，字段与 `PanelModelView` 一致（name/displayName/namespace/quant/sizeBytes/hostPort/status） |
| `GET /api/v1/models/:name` | 200 |
| `GET /api/v1/runtime/status` | 200，`{running:{model,displayName,container,startedAt,hostPort,ready,configStale}}` 与 `PanelRuntimeStatus` 一致；`ready` 为 llama-server 是否已监听 |
| `POST /api/v1/models/:name/start` | 200；不存在的模型 → **404**（与 `codeFor` 的 MODEL_NOT_FOUND 映射一致） |
| `GET /api/v1/proxy/llama/health` | 见下「就绪探测」 |
| 无 token 访问 | **401**（与 AUTH 映射一致） |
| `POST /api/v1/models/:name/start` 并发 | 第二个请求 → **409**（面板启停互斥），映射为 `RUNTIME_BUSY` 而非「面板不可达」 |
| `GET /api/v1/proxy/llama/v1/models` | 200，`data[].x_llamapad.reasoning_effort.{supported,levels}` 存在（无模型在跑时 503 → 插件归 null） |

**就绪探测语义（关键，此前未验证）**：模型加载中 `/api/v1/proxy/llama/health` 返回 **503** 而非 200，
`llamaHealth()` 的 `res.ok` 为 false → 正确继续轮询；就绪后转 200，且**此时立即发推理请求即成功**
（不早报）。27B 冷启动实测 **t+40s** 转 200，远低于 `startTimeoutMs` 默认 300s。

注意两种启动路径的耗时分布不同，插件均能正确处理：
- **冷启动**：`startModel()` 快速返回 → health 503 → 轮询至 200（27B 实测 40s）
- **热启动**（模型文件在 page cache）：`startModel()` 内部阻塞至就绪才返回（35B 实测约 31s）→ 返回后 health 立即 200

**校准项①usage 计数**：需请求带 `stream_options.include_usage`。末帧 usage 形如
`{"completion_tokens":1032,"prompt_tokens":21,"total_tokens":1053,"prompt_tokens_details":{"cached_tokens":0}}`
——`prompt_tokens_details.cached_tokens` 确实存在，口径疑问可关闭。

**校准项②reasoning_content**：`translate.ts` 的处理**正确无需改动**。实测一次带思考的对话：
779 帧 `delta.reasoning_content` → 249 帧 `delta.content`，切换点清晰，插件会先开 reasoning 块再开 text 块。

⚠️ 两个使用注意：
1. **是否产生 `reasoning_content` 取决于 llamapad 侧模型配置的 `enable_thinking`**（llamapad M4 已改为经容器
   env `LLAMA_CHAT_TEMPLATE_KWARGS` 注入，`--reasoning-format none` 被证伪不等价）。关掉思考则无 reasoning 块。
2. **`max_tokens` 给小了会全是 reasoning 没有 content**——实测 `max_tokens:300` 时思考未结束即截断，
   300 帧全为 reasoning_content、content 帧数为 0。dsh 侧配置 max tokens 时需给思考留出预算。

**校准项③切换延迟**：见上「就绪探测」。默认 `startTimeoutMs: 300000` 对 27B/35B 级别模型有充足余量。

**依赖的 llamapad 修复**：proxy 模式依赖 llamapad M4 的缺陷 #3 修复（`PANEL_LLAMA_HOST`，
面板容器内 127.0.0.1 不通向兄弟容器发布端口）。llamapad 早于该修复的版本，容器化部署下 proxy 模式不可用。


## 常见故障

| 现象 | 可能原因（错误码） |
|---|---|
| 启动即报 token 相关错误 | `AUTH`：token 无效/未授权（面板设置页重签） |
| 对话报「面板不可达」 | `PANEL_UNREACHABLE`：`panelUrl` 不通或面板进程挂了 |
| 对话报「模型不存在」 | `MODEL_NOT_FOUND`：cordis.yml 里 `model:` 与面板模型名不一致 |
| 对话报文件缺失 | `MODEL_FILES_MISSING`：面板里该模型的 gguf 文件不在盘上 |
| 长时间无响应后报超时 | `START_TIMEOUT`：模型加载超过 `startTimeoutMs`，调大该配置 |
| 对话报「还没启动」/「还没有模型在运行」（操作引导，指向设置 → 插件配置 → llamapad 模型面板） | `MODEL_NOT_RUNNING`：默认 `strict`（或 `passthrough`）档下，运行中模型与请求不一致，或面板未启动任何模型——设计行为，不是插件故障；去面板操作，或按需把 `chatBehavior` 改为 `auto-switch` |
