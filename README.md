...
实现，内容：工具呈现升级与并发安全声明、RPC 取消通道（卡片「取消等待」）、B 形态启停审批门、
多模态输入（贴图）、SSE 事件驱动刷新、GPU 监控页、系统提示本地模型场快照。多模型并行适配
（2026-09-21）也已全部实现并通过全部测试（618 单测 + 36 假面板 E2E）——提前适配 llamapad
面板尚未合并进 dev/main 的 `feature/multi-model` 分支（解除「同一时刻只运行一个模型」的
约束），本包的三档聊天路由、卡片、工具、系统提示快照均已按多模型语义改写，见下各节；真机
冒烟需等该面板分支合并后才能执行，清单见手册（只记清单，待执行）。**
...
多模型并行适配的计划与实施记录见
[docs/plans/2026-09-21-multi-model-adapt.md](docs/plans/2026-09-21-multi-model-adapt.md)；
...
- **面板可以同时运行多个模型**（提前适配 llamapad `feature/multi-model` 分支，该分支尚未
  合并进面板 dev/main）：面板 `runtime/status` 的 `running` 字段语义是**默认模型**（不带
  模型名的请求会打给它），真正的运行集合在 `models[]`；老面板没有这个字段时插件按「唯一
  运行的那个就是默认」向下兼容，行为与适配前完全一致
- **聊天路由 `chatBehavior` 三档**（决定「选模型」与「容器启停」的耦合程度，均已按多模型
  语义改写）：
  - `strict`（**默认**）：请求的模型只要**在运行集合里**（不要求是默认模型）就放行；一个
    模型都没跑、或目标没跑但别的在跑，一律报错引导去 llamapad 面板操作；聊天路径**完全不调
    start**，在途输出流绝对安全。适合共享 GPU / 多会话场景
  - `passthrough`：目标模型在跑就发给它；不在跑时发给**面板当前默认模型**（不是「随便一个
    在跑的」），没有默认模型时报错。适合「只想连上现在这个中转服务」的单人场景
  - `auto-switch`：只保证目标模型在跑，**绝不为了它停掉其他在跑的模型**（贴合插件「只做
    连接与调度，不接管服务端」的边界；`drainOnSwitch` 默认让服务端等待在途推理排空后再启动）。
    适合独占 GPU 的单人场景——多个会话各自 auto-switch 不同模型时，显存不够会启动失败，
    错误文案会列出当前还占着显存的其他模型，提示去面板手动停
- 插件内串行门 + 同目标合流避免并发抖动（仅 `auto-switch` 档会触发 start）
- **切换面板默认模型**：目标模型必须已在运行，走 `llamapad_set_default_model` 工具或设置
  卡片运行列表里的「设为默认」按钮（见下两节）；老面板没有这个能力（面板 `feature/multi-model`
  分支才有），会明确提示「需要面板多模型版本」
...
- **模型选择器上的运行状态标记**：运行中的模型名前会有 `▶︎` 前缀（实心播放三角，带
  U+FE0E 变体选择符强制取文本形态，避免在部分系统被渲染成彩色 emoji）；当前**默认模型**
  （不带模型名的请求会打给它）再加一个 `★` 前缀（两者可同时出现，如 `▶︎ ★ 某模型`），
  说明文字末尾追加「默认（不指定模型的请求发给它）」；`missing-file`/
...
锁，两个入口不会互相插队「一边起一边停」）。6 个工具：

| 工具 | 参数 | 说明 |
|---|---|---|
| `llamapad_status` | 无 | 面板是否可达、全部在跑模型逐行列出（各自是否就绪、是否为默认模型）；busy 探测仅针对默认模型，忙碌状态不可知时**省略**相关字段而不是报成"不忙" |
...
| `llamapad_start_model` | `model` 必填；`waitReady`/`drain`/`timeoutMs` 可选 | 启动/切换到目标模型：老面板会停旧起新；面板 `feature/multi-model` 分支下只确保目标在跑，**不驱逐其他在跑模型**（面板侧行为，插件原样透传）。`drain` 默认 `true`，切换不打断在途输出 |
| `llamapad_stop_model` | `model`/`drain`/`drainTimeoutMs` 均可选 | 停止指定模型；不传 `model` 时停**默认模型**。目标没在跑时返回 `stopped:false` 而不是报错，也绝不退而求其次去停别的模型 |
| `llamapad_set_default_model` | `model` 必填 | 切换面板默认模型（不带模型名的请求会打给它）；目标模型必须已在运行，否则面板拒绝（409）。需要面板多模型版本，老面板会明确提示「需要面板多模型版本」而不是报成别的错误 |

刻意**不开放**删除模型/文件、改配置、下载管理——高危操作留在 llamapad 面板的人工确认流程里。
工具失败直接抛错，由 dsh 转成模型可读的错误文本。

6 个工具里 5 个声明了 `isConcurrencySafe`（只读查询，或启停经共享门串行/合流），可被宿主安全地
并入并行组；`llamapad_set_default_model` 不声明（省略即按框架语义视为互斥——它改变的是全局
路由目标，不与其他调用无序交织）。调用与结果呈现：`llamapad_status` 是终端卡语义（等价一条
`llamapad status` 命令，面板不可达 → `exitCode: 1`，UI 用退出状态徽标呈现）；`llamapad_events`
的结果同样以终端卡输出多行事件（每行 `[时间] 类型 描述`，按时间倒序）；启停/切默认工具的
调用卡带 `execute` 标记（改变面板运行状态的操作）。

**启停/切默认审批门 `toolApproval`**（B 形态独有配置，默认 `allow`）：设为 `ask` 后，Agent 调用
`llamapad_start_model` / `llamapad_stop_model` / `llamapad_set_default_model` 需先经用户确认
（框架的 approval 通道）；宿主没有审批通道时，框架会把 `ask` 折算为**拒绝执行**——确认门绝不会
静默放行。共享 GPU 场景建议 `ask`（启停、切换默认模型都会影响别的会话）。
...
- 运行状态：只有一个模型在跑时是单行（是否正在推理）；**同时跑多个模型时改为运行列表**
  ——每行「模型名 · 已加载 N 秒 · ready 状态点 ·（默认标记 / 「设为默认」按钮）」，非默认
  行点「设为默认」即调用 `setDefaultModel` RPC 切换（在途禁用全部「设为默认」按钮，防止
  互相插队），失败沿用既有错误横幅；老面板不支持时报「当前面板版本不支持默认模型切换」
- 模型列表：卡片式两列网格，超过约 4 行时列表内部定高（320px）滚动，卡片本身不被撑高；
  dsh 窗口窄于 520px 时回落单列
- 每行一个启动/停止按钮（点启动即切换到该模型；面板 `feature/multi-model` 分支下切换
  不会驱逐其他在跑模型，语义同上「工作方式」的 `auto-switch` 档）；**在途等待时按钮变
  「取消等待」**——点击即中止这次等待（AbortSignal 经 host RPC 的取消通道一路传到面板
  HTTP 请求），按钮恢复原状、不报错误横幅（用户主动取消不算故障）
...
- 温度/功耗解析不到（nvidia-smi 不报）时该段省略而不是补 0——「65°C」和「温度未知」
  是两回事
- **多模型归属标注**（面板自身的采集器约束，插件改不了，只能如实告知）：容器与推理
  两张卡顶部标注「容器与推理指标来自默认模型 X」——面板的采集器只跟随**默认模型**，
  切换默认模型后曲线会在切换点直接拼接新模型的数据，**没有边界标记**；GPU 卡标注
  「整卡口径」——多个模型共享同一张卡时，nvidia-smi 只报得出整卡读数，无法拆分归因到
  具体某个模型

## 系统提示快照（混合路由）

插件向 dsh 的系统提示注入一个 `llamapad:local-fleet` 分节（英文，消费方是模型）：正在
跑什么模型（同时跑多个时逐个列出，默认模型标 `default`，并补一句「不指定模型的请求会
发给谁」）、还能启动哪些（可启动清单剔除全部在跑模型、至多 20 条，超出以计数提示）。任何
provider 的模型——包括云端模型——都能在系统提示里看到本地模型场，进而自行把合适的任务引导到本地
...
Agent 调用启停/切默认工具需用户确认，宿主无审批通道时折算为拒绝执行，见上「B 形态」）。
...
面板的档位声明只对**在跑的模型**有效（多模型面板下中转层给出的是运行集合的聚合列表，插件按
模型名精确匹配到目标自己的声明），因此没在跑的模型会列出完整枚举
`minimal / low / medium / high / xhigh / max`，启动之后再看到的才是该模型的真实值域。
...
- [多模型并行适配计划](docs/plans/2026-09-21-multi-model-adapt.md)——提前适配 llamapad
  `feature/multi-model` 分支，八个任务已全部实施
